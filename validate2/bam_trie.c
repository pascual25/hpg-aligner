...
void print_result(trie_result_t *result, int log) {
  ...
  FILE *wrong_mapped_file = fopen("wrong.mapped.ids", "w");
  FILE *not_mapped_file = fopen("not.mapped.ids", "w");
  ...
  for (int i = 0; i < num_reads; i++) {
	...
	fprintf(not_mapped_file, "%s\n", id);
	if (++limit_wrong_not_mapped < 10) {
  ...
  fclose(wrong_mapped_file);
  fclose(not_mapped_file);
