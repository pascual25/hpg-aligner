...
int apply_caling_bs_un(cal_seeker_input_t* input, batch_t *batch) {

  LOG_DEBUG("========= APPLY CALING BS UN =========\n");
  ...
  size_t *targets1 = mapping_batch->targets;
  size_t *targets2 = mapping_batch->targets2;
  ...
  size_t *targets;
  array_list_t *fq_batch1, *fq_batch2;
  ...
  for (int bs_id = 0; bs_id < 2; bs_id++) {
    ...
    for (size_t i = 0; i < num_targets; i++) {
      ...
      Ncg = mapping_batch->histogram_sw[target_index];
      Ngc = (bs_id == 0)?1 - Ncg:Ncg;
      //==========================================================================================
      LOG_DEBUG("--------- searching CALs -----------\n");

      read = array_list_get(target_index, fq_batch2);
      read2 = array_list_get(target_index, fq_batch1);
      list = mapping_lists[target_index];

      if (Ngc <= margen) {
	//max_seeds = (read->length / 15) * 2 + 10;
	num_cals = bwt_generate_cals_bs_un(read->sequence, read2->sequence, seed_size,
					   bwt_optarg, bwt_index2, bwt_index, list);
	// filter CALs
	list = filter_cals(num_cals, read->length, list);
	// and update targets
  ...
  LOG_DEBUG("========= END OF APPLYING CALING BS UN =========\n");

  if (batch->pair_input->pair_mng->pair_mode != SINGLE_END_MODE) {
    return BS_UN_PRE_PAIR_STAGE;
  } else if ( (batch->mapping_batch->num_targets  > 0) ||
	      (batch->mapping_batch->num_targets2 > 0)   ) {
